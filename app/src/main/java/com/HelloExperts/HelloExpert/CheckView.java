...
public  final class CheckView extends View {
    ...
    private static final String TAG = cdflynn.android.library.checkview.CheckView.class.getSimpleName();
    private static final boolean DEBUG = false;
    private static final long CHECK_ANIM_DURATION = 3000L;
    ...
    private void resolveAttributes(Context c, @Nullable AttributeSet attrs) {
        ...
        TypedArray a = c.getTheme().obtainStyledAttributes(attrs, cdflynn.android.library.checkview.R.styleable.CheckView, 0, 0);

        try {
            mStrokeWidth = a.getDimension(cdflynn.android.library.checkview.R.styleable.CheckView_checkView_strokeWidth, DEFAULT_STROKE_WIDTH);
            mStrokeColor = a.getColor(cdflynn.android.library.checkview.R.styleable.CheckView_checkView_strokeColor, DEFAULT_STROKE_COLOR);
        } finally {
            a.recycle();
        }
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (changed) {
            ...
            if (DEBUG && (mDrawingRect.width() != mDrawingRect.height())) {
                Log.w(TAG, "WARNING: " + cdflynn.android.library.checkview.CheckView.class.getSimpleName() + " will look weird because you've given it a non-square drawing area.  " +
    ...
    /**
     * Tell this {@link cdflynn.android.library.checkview.CheckView} to animate into the checked state.
     */
    public void check() {
        mChecked = true;
